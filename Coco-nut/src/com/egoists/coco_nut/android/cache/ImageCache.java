...
package com.egoists.coco_nut.android.cache;
...
import com.egoists.coco_nut.android.BuildConfig;
...
public class ImageCache {
    ...
    private static final int DEFAULT_MEM_CACHE_DIVIDER = 8; // memory class/this
                                                            // = mem cache size
    ...
    public static ImageCache findOrCreateCache(final FragmentActivity activity,
        ...
        final RetainFragment mRetainFragment = findOrCreateRetainFragment(activity
    ...
    private void init(Context context, ImageCacheParams cacheParams) {
        ...
        if (cacheParams.diskCacheEnabled) {
            ...
            if (getUsableSpace(diskCacheDir) > cacheParams.diskCacheSize) {
                try {
                    mICSDiskCache = ICSDiskLruCache.open(diskCacheDir, 1, 1,
            ...
            try {
                if (mICSDiskCache.get(key) == null) {
                    ...
                    if (editor != null) {
                        ...
                        bitmap.compress(mCacheParams.compressFormat, mCacheParams.compressQuality,
                                out);
                        editor.commit();
                    }
                }
            ...
            try {
                ...
                if (snapshot != null) {
                    ...
                    return BitmapFactory
                            .decodeStream(snapshot.getInputStream(ICS_DISK_CACHE_INDEX));
                }
            } catch (final IOException e) {
        ...
        if (mICSDiskCache != null) {
            try {
                if (!mICSDiskCache.isClosed()) {
                    // Should really close() here but need to synchronize up
                    // other methods that
